...
example7이 이 파일을 로드해서 reset 시 랜덤 샘플링.
"""
...
parser.add_argument("--upright_threshold", type=float, default=0.95,
                    help="박스 수직 정렬 최소값 (1.0=수직, 0.95≈18°이내). 이보다 기울어진 grasp 제외")
...
from source.example5_2.tasks.lift.lift_env_cfg import LiftEnvCfg
from source.example5_2.tasks.lift.lift_env import LiftEnv
...
def main():
    ...
    while (
        ...
        # 박스 upright score: local +z(박스 위쪽)이 world +z 방향 가리키는 정도
        # quat = [w, x, y, z], world_up_from_obj.z = 1 - 2*(x²+y²)
        upright_score = 1.0 - 2.0 * (obj_quat[:, 1] ** 2 + obj_quat[:, 2] ** 2)
        ...
        capture_mask = (
            ...
            & (r_to_obj < 0.15)
            & (gripper_close > 0.4)
            & (upright_score > args_cli.upright_threshold)
        ).cpu().numpy()
