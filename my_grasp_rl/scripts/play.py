"""SB3 PPO 공용 재생 스크립트 (grasp / stacking 공용)."""
...
parser.add_argument("--checkpoint", type=str, default=None, help="직접 체크포인트 경로 지정")
...
def main():
    ...
    # play는 보통 1개 env로 보는 게 편함
    if args_cli.num_envs is not None:
        cfg.scene.num_envs = args_cli.num_envs

    checkpoint_dir = "checkpoints"
    default_checkpoint_path = os.path.join(checkpoint_dir, f"{prefix}_ppo")
    vecnorm_path = os.path.join(checkpoint_dir, f"{prefix}_vecnormalize.pkl")

    checkpoint_path = args_cli.checkpoint if args_cli.checkpoint is not None else default_checkpoint_path
