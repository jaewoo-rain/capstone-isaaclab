...
class PlaceEnvCfg(DirectRLEnvCfg):
    ...
    # example7_2: edge-grasp 허용 (사용자 지시 — 중심 못 잡는 경우 있음)
    tilt_upright_threshold: float = 0.70    # 0.85→0.70 (~45° 허용, 비대칭 grip 인정)
    bonus_upright_threshold: float = 0.95   # 0.985→0.95 (~18°, 보너스 임계 완화)
    on_floor_z_threshold: float = 0.10      # 물체 중심 z가 이 이하면 바닥 판정
    abandoned_dist_threshold: float = 0.20  # 격벽 충돌 시 튕김 허용
    ...
    # example7 (xy aligned at z=0.30) 의 success state 에서 수집
    handoff_dataset_path: str = "checkpoints/handoff_xy_aligned.npz"
