...
#     #     dist = torch.norm(self.obj_to_grip, dim=-1)
...
#     #     # 2) 접근 진행 보상
#     #     approach_reward = torch.clamp(self._prev_dist - dist, min=0.0) * 10.0
...
#     #         + self._robot.data.joint_pos[:, self.right_finger_joint_id]
#     #     )
#     #     horizontal_dist = torch.norm(self.obj_to_grip[:, :2], dim=-1)
#     #     is_grasping = (horizontal_dist < 0.04) & (gripper_width < 0.06)
#     #     grasp_bonus = is_grasping.float() * 2.0

#     #     # 4) 리프트 보상
#     #     obj_height = self.obj_pos_w[:, 2]
#     #     lift_reward = torch.clamp(obj_height - 0.04, min=0.0) * 15.0
...
#     #     # ── callback이 직접 읽을 수 있도록 reward_log 갱신 ──
...
#         # 정렬 완료 판정
#         xy_aligned = xy_dist < 0.035
#         z_aligned = z_dist < 0.035
...
#         # pre-grasp 정렬 보상
#         pre_grasp_reward = aligned.float() * 2.0
...
#         finger_score = torch.clamp(0.05 - left_to_obj, min=0.0) + torch.clamp(0.05 - right_to_obj, min=0.0)
#         close_reward = alignment_score * finger_score * torch.clamp(0.08 - gripper_width, min=0.0) * 20.0
