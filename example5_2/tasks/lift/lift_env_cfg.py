...
class LiftEnvCfg(DirectRLEnvCfg):
    ...
    sim: SimulationCfg = SimulationCfg(
        ...
        physics_material=sim_utils.RigidBodyMaterialCfg(
            ...
            # 동적마찰: 물체가 움직이는 중 저항. 3.0 = 매우 강한 저항 (박스 미끄러짐 방지)
    ...
    # 물체 높이가 이 값을 넘으면 lift 성공으로 판정
    # v26 curriculum 1단계: 0.20 → 0.12 (박스 6cm 들면 성공, 첫 success 유발)
    # 60% 달성 후 0.20으로 복원 예정 (curriculum 2단계)
    ...
    # 0.02 = 박스 위쪽 1/3 지점 (z=0.08m). 윗부분(0.04) 대비 회전 모멘트 ↓, 정중앙(0.0) 대비 바닥 충돌 회피
    grasp_target_z_offset: float = 0.02
    ...
    # 0.005 = 약한 탐색 장려
    ent_coef: float = 0.005  # v47: pure grasp 학습이라 exploration 강제 불필요, 원복

    # v47: grasp + N-step hold task
    # success = grasp 조건 만족하며 N step 연속 유지
    grasp_hold_steps: int = 60  # 1초 (60Hz × 1s)
