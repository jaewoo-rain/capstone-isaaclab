...
class PlaceEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ===========================================================
        # example7_2 = Yaw + Descent + Insert (2026-05-06 redesign)
        # 입력: example7 의 success state (xy aligned at z=0.30, gripped, yaw 임의)
        # 목표: yaw 맞추고 셀 안에 적재
        # 성공: xy<2.5cm AND yaw<2cm AND on_floor
        # ===========================================================
        success = xy_aligned & endpoint_aligned & on_floor

        # ── 6-Tier Reward (all-positive) ─────────────────────────
        # T0: XY 유지 (sharp, 시작 시점 이미 xy 정렬됨)
        r_xy = torch.exp(-50.0 * xy_dist ** 2)

        # T1: Yaw 정렬 (endpoint 단일축, sharp)
        r_yaw = torch.exp(-50.0 * endpoint_single_axis_diff ** 2)

        # T2: 하강 (xy + yaw 정렬 시점부터 활성, 셀 바닥 향해)
        z_target = obj_height * 0.5
        ready_to_descend = xy_aligned & endpoint_aligned
        r_descent = torch.where(
            ready_to_descend,
            torch.exp(-30.0 * (hover_z - z_target) ** 2),
            torch.zeros_like(hover_z),
        )

        # T3: 잡기 유지 (떨어뜨리지 말기, 가벼운 가중치)
        ...
        reward = (
              80.0 * r_xy             # 0~80, xy 유지
            + 100.0 * r_yaw            # 0~100, yaw 정렬 (핵심)
            + 100.0 * r_descent        # 0~100, 하강 (xy+yaw 후)
            +  30.0 * r_grip_holding   # 0~30, 떨어뜨리지 말기
            +  20.0 * r_upright        # 0~20
        ...
        r_endpoint_aligned_gated = ready_to_descend.float()
        ...
        xy_upright_ready = ready_to_descend.float()
        ...
        self._last_inserted = on_floor                      # = on floor (drop)
        self._last_xy_aligned_loose = xy_aligned_loose
        self._last_yaw_aligned = endpoint_aligned
        ...
        self.reward_log["rew_t0_pull"] = float(r_pull.mean())
        self.reward_log["rew_t1_xy"] = float(r_xy.mean())
        self.reward_log["rew_t2_descent"] = float(r_descent.mean())
        self.reward_log["rew_t3_upright"] = float(r_upright.mean())
        self.reward_log["rew_t4_success"] = float(r_success.mean())
        self.reward_log["rew_total"] = float(reward.mean())

        # 새 tier 로그 (Yaw + Insert task)
        self.reward_log["rew_n0_xy"] = float(r_xy.mean())
        self.reward_log["rew_n1_yaw"] = float(r_yaw.mean())
        self.reward_log["rew_n2_descent"] = float(r_descent.mean())
        ...
        self.reward_log["rate_yaw_aligned"] = float(endpoint_aligned.float().mean())
        self.reward_log["rate_ready_to_descend"] = float(ready_to_descend.float().mean())
