...
def fold_yaw_sym(angle: torch.Tensor) -> torch.Tensor:
    """박스 180° 대칭(직사각형 단면) → yaw 오차를 ±90°로 fold. 180° 차이 = 정렬됨."""
...
class InsertEnv(DirectRLEnv):
    ...
    def _get_observations(self) -> dict:
        ...
        slot_yaw_err = fold_yaw_sym(self._cell_yaw - cur_ee_yaw)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        cur_ee_yaw = self._extract_ee_yaw()
        yaw_err = fold_yaw_sym(self._cell_yaw - cur_ee_yaw)
        ...
        # 비포화 linear yaw cone — 80°(1.4rad)에서도 일정 복구 gradient (Gaussian saturation 탈출).
        # yaw_err 은 fold_yaw_sym 으로 ±π/2 유계 → cone ∈ [0, w], all-positive 유지.
        r_yaw_lin = self.cfg.reward_yaw_lin_w * (1.0 - yaw_err.abs() / (math.pi / 2.0))
        ...
        self.reward_log = {
            ...
            "r_yaw_lin": float(r_yaw_lin.mean().item()),
