...
class PlaceEnv(DirectRLEnv):
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # ===========================================================
        # T1: XY 가까이 (가우시안)
        r_xy_close = torch.exp(-25.0 * xy_dist ** 2)

        # T1b: Z를 목표 높이로 (가우시안, 별도 항목)
        target_z = 0.30
        z_diff = hover_z - target_z
        r_z_close = torch.exp(-30.0 * z_diff ** 2)

        # T3: 중심 정렬 AND 끝점 단일축 정렬 (bool, 합성)
        # T1 가우시안이 정렬 보상을 흡수하므로 T2(bool 보너스) 제거
        r_endpoint_aligned = (xy_aligned & endpoint_aligned).float()

        # T4: 성공 (중심 정렬 + 바닥 접촉)
        r_success = success.float()

        # T5: 그리퍼-물체 가까이 유지 (잡고 있기 유도, 가파른 가우시안)
        r_grip_near_obj = torch.exp(-50.0 * grip_to_obj_dist ** 2)

        # T6: 물체 근처에서 그리퍼 닫혀있으면 보너스 (성공 후엔 비활성)
        near_obj = grip_to_obj_dist < 0.06
        closed = gripper_close_state > 0.5
        r_keep_closed = (near_obj & closed & ~success).float()

        # T7: 물체 수직 유지 (1.0=완벽 수직, 0.0=옆으로 누움)
        # clamp(0,1)로 음수(뒤집힘) 방지
        r_upright = torch.clamp(upright_score, 0.0, 1.0)

        # T8: 기울어지지 않음 보너스 (bool, 엄격)
        # 기울임 ≤ 10° (upright_score > 0.985) 일 때 +1, 거의 완벽 수직일 때만 보너스
        r_not_tilted = (upright_score > self.cfg.bonus_upright_threshold).float()

        # T9: 기울어짐 페널티 (bool, 관대)
        # 기울임 > 30° (upright_score < 0.85) 일 때 +1 → reward에서 빼므로 페널티
        r_severe_tilt = tilted.float()

        # T3 게이팅: xy 정렬 + 수직이 거의 완성된 다음에 yaw(끝점) 정렬 작업
        # r_xy_close × r_upright 가 충분히 높을 때만(0.8+) endpoint 정렬 활성화
        xy_upright_ready = ((r_xy_close * r_upright) > 0.8).float()
        r_endpoint_aligned_gated = r_endpoint_aligned * r_upright * xy_upright_ready

        # T10: 위에서 접근 보상 (페널티 → 보상 형태로 전환)
        # 정렬(5cm) 안 됐을 때, 적정 높이 [0.20, 0.40] 유지하면 보상
        # 너무 낮아도 (셀 격벽 충돌 위험), 너무 높아도 (의미 없음) 보상 없음
        safe_height = (~xy_aligned_loose) & (hover_z >= 0.20) & (hover_z <= 0.40)
        r_above_when_unaligned = safe_height.float()

        # T11: 너무 빠른 움직임 페널티 (action L2 norm)
        # 부드러운 움직임 유도, 박스 놓침 방지
        action_penalty = torch.sum(self.actions ** 2, dim=-1)

        reward = (
            300.0 * r_xy_close                    # T1: XY 가우시안 (정렬 신호)
            + 50.0 * r_z_close                    # T1b: Z 가우시안 (목표 높이 유지)
            + 80.0 * r_endpoint_aligned_gated     # T3: 끝점 정렬 (xy+upright 완성 후만)
            + 500.0 * r_success                   # T4: 성공 (압도적)
            + 10.0 * r_grip_near_obj              # T5: 잡기 유지 (가우시안)
            + 30.0 * r_keep_closed                 # T6: 그리퍼 닫혀있기
            + 150.0 * r_upright                   # T7: 수직 유지 (continuous)
            + 100.0 * r_not_tilted                # T8: 기울어지지 않음 보너스 (≤10°)
            - 100.0 * r_severe_tilt               # T9: 기울어짐 페널티 (>30°)
            + 30.0 * r_above_when_unaligned       # T10: 위에서 접근 보상 (정렬 전 안전 높이 유지)
            - 0.5 * action_penalty                # T11: 부드러운 움직임 (action L2 페널티)
        )
